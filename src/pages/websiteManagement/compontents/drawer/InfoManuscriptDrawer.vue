      ...
      <a-row :gutter="[0, 48]">
        <a-col :span="24">
          ...
          <div class="noticeTime">
            <div>来源：{{ currentRecord.newsfrom }}</div>
          </div>
...
export default defineComponent({
  setup() {
    ...
    onBeforeUnmount(() => {
      EventBus.off('handleViewNews')
    })

    const EventBusListener = () => {
      EventBus.on('handleViewNews', (record) => {
        ...
        if (currentRecord.value.attachments) {
         ...
         if (currentRecord.value.newscontent) {
          currentRecord.value.newscontent =   currentRecord.value.newscontent.replace(
            /@serveruirl@LEAP/g,
            window.location.origin + "/NVSIDF/LEAP"
          );
        }
        visible.value = true
      })
