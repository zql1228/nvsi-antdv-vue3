    ...
    >
      <a-form :label-col="labelCol" :wrapper-col="wrapperCol" ref="formRef" :model="form" :rules="rules">
        <a-row :gutter="24">
          ...
          <!-- 为2级栏目的时候才能选择是否为导航 -->
          <a-col :span="24" >
            <a-form-item name="isnav" label="是否为导航">
              <a-radio-group v-model:value="form.isnav" :options="optionsList.options4" :disabled="props.item == 'view'"></a-radio-group>
            </a-form-item>
          </a-col>

          <a-col :span="24">
            <a-form-item name="pagetype" label="页面类型">
              <a-radio-group v-model:value="form.pagetype" :options="optionsList.options3" :disabled="props.type == 'view'"></a-radio-group>
            </a-form-item>
          </a-col>
          ...
          <a-col :span="24">
            <a-form-item name="isquote" label="是否引用总站">
              <a-radio-group v-model:value="form.isquote" :options="optionsList.options5" :disabled="props.type == 'view'"></a-radio-group>
            </a-form-item>
          </a-col>
          <a-col :span="24">
            <a-form-item name="icon" label="图片封面">
              <div class="clearfix">
                <img v-if="imageUrl" :src="imageUrl" @click="deleteImg" />
                <a-upload :file-list="fileList2" list-type="picture-card"
                 :beforeUpload="beforeUpload" :customRequest="handleFileUpload"
                 :disabled="props.type == 'view'"
                 >
                  <div v-if="fileList2.length < 1 && !imageUrl">
                    <plus-outlined />
                    <div style="margin-top: 8px">上传导航图标</div>
                  </div>
                </a-upload>
              </div>
            </a-form-item>
      ...
      >
        <!-- 新增查看修改操作 -->
        <a-button type="primary" @click="submit(0)" :loading="loading1" v-show="props.type != 'view'">{{
...
export default defineComponent({
  ...
  setup(props, text) {
    ...
    const submit = (sendstatus) => {
      ...
      formRef.value.validate().then(() => {
        let bean = {
          ...
          parentnode: props.type == 'modify' ? props.item.parentnode : props.item,
          params: form.params,
          sort: form.sort,
    ...
    const rules = reactive({
      ...
      pagetype: [{ required: true, message: '页面类型为空', trigger: 'change' }],
      isshow: [{ required: true, message: '是否显示为空', trigger: 'change' }],
      sort: [{ required: true, message: '排序为空', trigger: 'change' }],
      isquote: [{ required: true, message: '是否引用总站为空', trigger: 'change' }],
    })
