      ...
      <template #table>
        <a-tabs type="card" v-model:activeKey="activeKey">
          <a-tab-pane key="1" tab="正常">
            <TeamQueryTable ref="table1" :queryParam="queryParam" activeKey="1">
              <template #action="record">
                <a-space>
                  <a-button size="small" @click="edit(record.record)"> 修改信息 </a-button>
                  <a-button size="small" type="primary" @click="updatePassword(record.record)"> 重置密码 </a-button>
                  <!-- :disabled="level == '全国'" -->
                  <a-button
                    type="primary"
                    size="small"
                    @click="handleEdit(record.record, 0)"
                    :disabled="record.record.albe0026 != 1 && record.record.albe0026 != 3"
                  >
                    <!-- :disabled="(record.record.albe0026 != 1 && record.record.albe0026 != 3) || level == '全国'" -->
          ...
          <a-tab-pane key="2" tab="未审核">
            <TeamQueryTable ref="table2" :queryParam="queryParam" activeKey="2">
              <template #action="record">
                <a-space>
          ...
          <a-tab-pane key="3" tab="已停用">
            <TeamQueryTable ref="table3" :queryParam="queryParam" activeKey="3">
              <template #action="record">
                <a-space>
                  ...
                  <a-button
                    type="primary"
                    size="small"
                    @click="handleEdit(record.record, 1)"
                    :disabled="record.record.albe0026 != 1 && record.record.albe0026 != 3"
                  >
                    <!-- :disabled="(record.record.albe0026 != 1 && record.record.albe0026 != 3) || level == '全国'" -->
                  ...
                  <a-button
                    type="primary"
                    size="small"
                    @click="handleEdit(record.record, 2)"
                    :disabled="record.record.albe0026 != 1 && record.record.albe0026 != 3"
                  >
                    <!-- :disabled="(record.record.albe0026 != 1 && record.record.albe0026 != 3) || level == '全国'" -->
...
import TeamQueryTable from './table/TeamQueryTable.vue'
...
import { checktime2, getLevel } from '@/utils/publicMethods/common'
...
export default defineComponent({
  components: { DownOutlined, UpOutlined, TeamQueryTable, modal, TeamQueryDrawer, updatePasswordModal },
  setup() {
    ...
    const updatePassword = (record, type) => {
      //重置密码按钮
    ...
    const onClose3 = (bool) => {
      //重置密码弹窗关闭
      if (bool) {
        activeKey.value == 1 ? table1.value.getList(false) : activeKey.value == 2 ? table2.value.getList(false) : table3.value.getList(false)
      }
