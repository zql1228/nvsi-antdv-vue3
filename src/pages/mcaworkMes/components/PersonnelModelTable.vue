...
export default defineComponent({
  ...
  setup(props, text) {
    ...
    const onClose = () => {
      text.emit('child', state.selectedRowKeys, state.personnelList)
    }
    const searchFunc = () => {
      return {
    ...
    const onSelect = (record, selected, selectedRows, nativeEvent) => {
      //用户手动选择/取消选择某列的回调
      if (selected) {
        state.personnelList.push(record)
      }

      if (!selected) {
        const delIndex = state.personnelList.findIndex((val) => {
          return val.id === record.id
        })
        state.personnelList.splice(delIndex, 1)
      }
    }
    ...
    const onSelectAll = (selected, selectedRows, changeRows) => {
      //用户手动选择/取消选择所有列的回调
      if (selected) {
        state.personnelList = state.personnelList.concat(changeRows)
      }
      if (!selected) {
        let selectedRows = JSON.parse(JSON.stringify(state.personnelList))
        const delIndex = []
    ...
    watch(
      ...
      (value) => {
        ...
        state.personnelList = props.personnelList
      }
    )
