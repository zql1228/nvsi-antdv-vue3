          ...
          <a-row :gutter="24">
            ...
            <!-- <a-col :md="6" :sm="24">
                <a-form-item label="项目状态" name="albx0072">
                  <a-select v-model:value="queryParam.albx0072" :options="optionsList.albx0072List" />
                </a-form-item>
              </a-col> -->
      ...
      <template #table>
        <a-tabs type="card" v-model:activeKey="activeKey">
          <a-tab-pane key="1" tab="运行中">
            <NationalProjectQueryTable ref="table1" :queryParam="queryParam" activeKey="1">
              <template #action="{ record }">
                <a-space>
                  <a-popconfirm :title="'确定停用项目' + record.albx0002 + '吗?'" @confirm="handleAdd(record, 1)" disabled>
                    <a-button type="primary" size="small" disabled> 停用 </a-button>
                  </a-popconfirm>
                  <a-button type="primary" ghost size="small" @click="handleEdit(record)" disabled> 操作记录 </a-button>
                </a-space>
              </template>
            </NationalProjectQueryTable>
          </a-tab-pane>
          <a-tab-pane key="2" tab="已结项">
            <NationalProjectQueryTable ref="table2" :queryParam="queryParam" activeKey="2">
              <template #action="{ record }">
                <a-space>
                  <a-button type="primary" ghost size="small" @click="handleEdit(record)" disabled> 操作记录 </a-button>
                </a-space>
          ...
          <a-tab-pane key="3" tab="已停用">
            <NationalProjectQueryTable ref="table3" :queryParam="queryParam" activeKey="3">
              <template #action="{ record }">
                <a-space>
                  <a-popconfirm :title="'确定启用项目' + record.albx0002 + '吗?'" @confirm="handleAdd(record, 2)" disabled>
                    <a-button type="primary" size="small" disabled> 启用 </a-button>
                  </a-popconfirm>
                  <a-button type="primary" ghost size="small" @click="handleEdit(record)" disabled> 操作记录 </a-button>
                </a-space>
              </template>
...
import NationalProjectQueryTable from './compontents/NationalProjectQueryTable'
import apis from '@/server/request'
export default defineComponent({
  components: { DownOutlined, UpOutlined, QueryDrawerTable, QueryModelTable, NationalProjectQueryTable },
  setup() {
    ...
    const handleAdd2 = (record, type) => {
      //停用
      let bean = {
        areaid: record.albx0021,
        type: type == '1' ? 2 : 1, //type 1停用 2启用
        id: record.id,
